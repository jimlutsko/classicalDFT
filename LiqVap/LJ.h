#ifndef __LJEMPIRICAL__
#define __LJEMPIRICAL__
...
{
 public:
 JZG1(double rc = -1.0, bool doShift = true)
   : gamma(3.0), da_(0.0)
    {
  ...
  virtual double freeEnergy(double density, double kT)
  {
    double f  = kT*(log(density)-1);
    for(int i=1;i<=8;i++) f += a(kT,i)*pow(density,i)/i;
    for(int i=1;i<=6;i++) f += b(kT,i)*G(density,i);
    f += da_*density;
    return f/kT;
  }
  virtual double Pressure(double density, double kT)
  {
    double P  = density*kT;
    double F = exp(-gamma*density*density);
    for(int i=1;i<=8;i++) P += a(kT,i)*pow(density,i+1);
    for(int i=1;i<=6;i++) P += F*b(kT,i)*pow(density,2*i+1);
  ...
  virtual double phi1x(double density, double kT) const 
  { 
      double F = exp(-gamma*density*density);
  ...
  virtual double phi2x(double density, double kT) const
  { 
      double F = exp(-gamma*density*density);
  ...
  virtual double phi3x(double density, double kT) const 
  { 
      double F = exp(-gamma*density*density);
 ...
 private:
  ...
  {
    if(i == 1) return x1*T+x2*sqrt(T)+x3+(x4/T)+x5/(T*T);
    if(i == 2) return x6*T+x7+(x8/T)+x9/(T*T);
    if(i == 3) return x10*T+x11+(x12/T);
    if(i == 4) return x13;
    if(i == 5) return (x14/T)+x15/(T*T);
    if(i == 6) return x16/T;
    if(i == 7) return (x17/T)+x18/(T*T);
    if(i == 8) return x19/(T*T);
...
{
 public:
 LJMecke(double rc = -1.0, bool doShift = true)
   : rhoc_(0.3107), kTc_(1.328), da_(0.0)
    {
  ...
  virtual double freeEnergy(double rho, double kT)
  {
    double s = 0.1617*(rho/rhoc_)/(0.689+0.311*pow(kT/kTc_,0.3674));
    double fid = log(rho)-1;
    double fhs = (4*s-3*s*s)*pow(1-s,-2);
    double fa  = 0.0;
    
    for(int i=0;i<32;i++)
      fa += c_[i]*pow(kT/kTc_,m_[i])*pow(rho/rhoc_,n_[i])*exp(p_[i]*pow(rho/rhoc_,q_[i]));
  ...
  // P/NkT
  virtual double Pressure(double rho, double kT)
  {
    double s = 0.1617*(rho/rhoc_)/(0.689+0.311*pow(kT/kTc_,0.3674));
    double pid = 1.0;
	...
	*c_[i]*pow(kT/kTc_,m_[i])*pow(rho/rhoc_,n_[i])*exp(p_[i]*pow(rho/rhoc_,q_[i]));
...
{
 public:
 LJ_FMSA(double rc = -1.0, bool doShift = true)
   : da_(0.0)
    {
      if(rc > 0.0)
	{
  ...
  virtual double freeEnergy(double rho, double kT)
  {
    ...
    double d = (1+0.2977*kT)/(1+0.33163*kT+1.0477*(1e-3)*kT*kT);
    ...
    double phi_0 = -(k1/kT)*L1*pow(1-e,-2)/(Q1*z1*z1);
    phi_0 -= -(k1/kT)*(1+z1*d)/(z1*z1);
    phi_0 -=  -(k2/kT)*L2*pow(1-e,-2)/(Q2*z2*z2);
    phi_0 += -(k2/kT)*(1+z2*d)/(z2*z2);
    
    phi_0 += (4/kT)*((1.0/9)*pow(d,-9)-(1.0/3)*pow(d,-3));
    phi_0 -= (4/kT)*g0*((1.0/9)*pow(d,-9)-(1.0/3)*pow(d,-3)+(2.0/9));
        
    double phi_1 = -(k1*k1/(kT*kT))/(2*z1*Q1*Q1*Q1*Q1);
    phi_1 -= (k2*k2/(kT*kT))/(2*z2*Q2*Q2*Q2*Q2);
    phi_1 += 2*(k1*k2/(kT*kT))/((z2+z1)*Q1*Q1*Q2*Q2);
    
    phi_1 -= (4/kT)*(d*d*d)*((1.0/(Q1*Q1*T1))-(1.0/(Q2*Q2*T2)))*((1.0/9)*pow(d,-9)-(1.0/3)*pow(d,-3)+(2.0/9));

    return phi_cs+4*M_PI*0.5*rho*(phi_0+0.5*phi_1) + da_*rho/kT;
  }
  // P/NkT
  virtual double Pressure(double rho, double kT)
  {
    ...
    double phi_0 = 0.0;
    phi_0 -= (k1/kT)*L1*pow(1-e,-2)/(Q1*z1*z1);
    phi_0 -= -(k1/kT)*(1+z1*d)/(z1*z1);
    phi_0 -= -(k2/kT)*L2*pow(1-e,-2)/(Q2*z2*z2);
    phi_0 += -(k2/kT)*(1+z2*d)/(z2*z2);
    ...
    double phi_01 = 0;
    phi_01 -= (k1/kT)*L11*pow(1-e,-2)/(Q1*z1*z1);
    phi_01 -= 2*(k1/kT)*L1*pow(1-e,-3)/(Q1*z1*z1);
    phi_01 -= -Q11*(k1/kT)*L1*pow(1-e,-2)/(Q1*Q1*z1*z1);

    phi_01 -= -(k2/kT)*L21*pow(1-e,-2)/(Q2*z2*z2);
    phi_01 -= -2*(k2/kT)*L2*pow(1-e,-3)/(Q2*z2*z2);
    phi_01 -= Q21*(k2/kT)*L2*pow(1-e,-2)/(Q2*Q2*z2*z2);

    phi_01 -= (4/kT)*g01*((1.0/9)*pow(d,-9)-(1.0/3)*pow(d,-3)+(2.0/9));
    
    double phi_1 = 0.0;
    phi_1 -= (k1*k1/(kT*kT))/(2*z1*Q1*Q1*Q1*Q1);
    phi_1 -= (k2*k2/(kT*kT))/(2*z2*Q2*Q2*Q2*Q2);
    phi_1 += 2*(k1*k2/(kT*kT))/((z2+z1)*Q1*Q1*Q2*Q2);
    ...
    double phi_11 = 0.0;
    phi_11 -= -4*Q11*(k1*k1/(kT*kT))/(2*z1*Q1*Q1*Q1*Q1*Q1);
    phi_11 -= -4*Q21*(k2*k2/(kT*kT))/(2*z2*Q2*Q2*Q2*Q2*Q2);
    phi_11 += -2*Q11*2*(k1*k2/(kT*kT))/((z2+z1)*Q1*Q1*Q1*Q2*Q2);
    phi_11 += -2*Q21*2*(k1*k2/(kT*kT))/((z2+z1)*Q1*Q1*Q2*Q2*Q2);
    
    phi_11 -= (4/kT)*(d*d*d)*((-2*Q11/(Q1*Q1*Q1*T1))-(-2*Q21/(Q2*Q2*Q2*T2)))*((1.0/9)*pow(d,-9)-(1.0/3)*pow(d,-3)+(2.0/9));

    return P_cs+4*M_PI*0.5*rho*(phi_0+0.5*phi_1)+4*M_PI*0.5*rho*e*(phi_01+0.5*phi_11) + da_*rho/kT;
  }
