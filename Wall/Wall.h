...
class Wall : public Density
{
	...
	for(int iz=0;iz<Nz_;iz++)
	  {
	    ...
	    if(z >= zwall-hsd_/2 || z <= -zwall+hsd_/2) V = 1000000;
	    else V = 0;
	    ...
	    vWall_.set(pos(ix,iy,iz),V);
	  }

    cout << "Z(0) = " << getZ(0) << " zwall = " << zwall << " so infinite for z < " << -zwall+hsd_/2 << endl;
  }
  ...
  */  
  ...
  virtual void doDisplay(string &title, string &file, int seq) const
  {
    ...
    for(int i= 0; i < Nz(); i++)
      {
	double x = getZ(i)-(getZ(0)+1);
	g_->addPoint(x,(M_PI*hsd_*hsd_*hsd_/6)*getDensity(Nx()/2,Ny()/2,i),seq);
	...
	*/
    ...
    cout << "emax = " << emax << " Natoms = " << getNumberAtoms() << endl;
      g_->setTitle(title.c_str());
    //    g_->setColor(2,1);
      //      g_->setLimits(getZ(0)+1,getZ(0)+4,0,1);
      //      g_->setLimits(0,4,0,1);
      //            g_->setLimits(0,40,0,0.6);
    g_->redraw(0);
  }  
