#ifndef __LUTSKO_POTENTIAL2__
#define __LUTSKO_POTENTIAL2__
...
{
 public:
 Potential2(double sigma, double eps, double rcut) : sigma_(sigma), eps_(eps),rcut_(rcut)
  {
    shift_ = vr(rcut_);
    rmin_  = pow(2.0,1.0/6.0)*sigma;
    Vmin_  = V(rmin_);
    cout << "rmin_ = " << rmin_ << " Vmin_ = " << Vmin_ << endl;
  }
  ...
  {
    kT_ = kT; 
    Integrator<const Potential2> II(this, &Potential2::dBH_Kernal, 1e-4, 1e-6);
