	...
	for(int iz = -Nz_lim;iz<=Nz_lim; iz++)
	  {
		...
		double x;
		double wx;
		gsl_integration_glfixed_point(0, 1, Gx, &x, &wx, tr);		
		for(int Gy=0; Gy < Ngauss_; Gy++)
		  {
		    double y;
		    double wy;
		    gsl_integration_glfixed_point(0, 1, Gy, &y, &wy, tr);				    
			...
			for(int i=-1;i<=1;i++)
			  for(int j=-1;j<=1;j++)
				...
				for(int b=-1;b<=1;b+=2)
				  for(int c=-1;c<=1;c+=2)
				    {
				      int nnx = nx-i-(a == -1? 1 : 0);
				      int nny = ny-j-(b == -1? 1 : 0);
				      int nnz = nz-k-(c == -1? 1 : 0);
				      ...
				      double fy = wy*(1-y)*((1-y-y*y)*(i == 0 ? 2 : -1)+2-3*i*b*y);
				      double fz = wz*(1-z)*((1-z-z*z)*(k == 0 ? 2 : -1)+2-3*k*c*z);

				      double w = fx*fy*fz*watt;    
