...
void DDFT_Discrete::sub_step_x(DFT_Vec &ynew, const Density& original_density, bool bFixedBoundaries)
{
...
void DDFT_Discrete::sub_step_y(DFT_Vec &ynew, const Density &original_density, bool bFixedBoundaries)
{
  double dy = original_density.getDY();
  double Dy = dt_/(dy*dy);
...
bool DDFT_Discrete::sub_step_z(DFT_Vec &ynew, const Density &original_density, bool bUseForces, bool bFixedBoundaries) //const DFT_Vec &oldF)
{
  double dx = original_density.getDX();
  double dy = original_density.getDY();
  double dz = original_density.getDZ();
...
#pragma omp parallel for			\
  ...
  double DDFT_Discrete::step_string(double &dt, Density &original_density, double self_consistency_threshold)
  {
    dt_ = dt;
...
void DDFT_Discrete::solv_tridiag(const DFT_Vec &b, DFT_Vec &RHS, double D, bool bFixedBoundaries)
  {
    int N = RHS.size();
...
void DDFT_Discrete::solv_periodic_tridiag_2(DFT_Vec &b, DFT_Vec &RHS, double D)
{
  int N = RHS.size();
  ...
  void DDFT_Discrete::solv_periodic_tridiag(DFT_Vec &RHS, double D)
  {
    int N = RHS.size();
...
void DDFT_Discrete::test_solv_tridiag()
{
  int N = 8;
