...
double DDFT_Open::step_string(double &dt, Density &original_density, unsigned &time_den, bool verbose)
{
...
double DDFT_Open::step()
{
  ...
  do {
    ...
    if(deviation > tolerence_fixed_point_)
      throw std::runtime_error("Failed to find fixed point in DDFT_Open::step()");
  } while(reStart);
...
void DDFT_Open::calcNonlinearTerm(const DFT_Vec &d2, const DFT_Vec &dF, DFT_Vec &RHS1)
{
  double dx = density_.getDX();
  double dy = density_.getDY();
  double dz = density_.getDZ();
